...
function getExecPath(commandName, onError, versionArg, inBinDir) {
    if (onError === void 0) { onError = function (error) { return console.log(error); }; }
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    ...
                    versionRegexp = /\d+\.\d+\.\d+/;
                    return [4 /*yield*/, spawnCommand("\"" + buildCommand + "\"", [versionArg], onError)];
                case 1:
                ...
                case 2:
                    ...
                    return [4 /*yield*/, spawnCommand("\"" + pathCommand + "\"", [versionArg], onError)];
                case 3:
                    pathVersion = _a.sent();
...
function spawnCommand(command, args, onError) {
    if (onError === void 0) { onError = function (error) { return console.log(error); }; }
    return new Promise(function (resolve, reject) {
        var cp = child_process_1.spawn(command, args, { windowsHide: true, shell: true });
        var outBuffers = [];
        var errBuffers = [];
        cp.stdout.on('data', function (b) { return outBuffers.push(b); });
        cp.stderr.on('data', function (b) { return errBuffers.push(b); });
        ...
        cp.on('exit', function (code, signal) {
            if (code === 0) {
                var result = Buffer.concat(outBuffers).toString('utf8').trim();
                resolve(result);
                return;
            }
            if (errBuffers.length > 0) {
                var message = Buffer.concat(errBuffers).toString('utf8').trim();
                var error = new Error("Error executing " + command + " " + args.join(' ') + ": " + message);
                onError(error);
                reject(error);
                return;
            }
            if (signal) {
                var error = new Error("Process exited with signal: " + signal);
                onError(error);
                reject(error);
                return;
            }
            if (code) {
                var error = new Error("Process exited with exit code: " + code);
                onError(error);
                reject(error);
                return;
            }
